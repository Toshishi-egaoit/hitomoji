# 日本語IME「ひともじ」

ひともじ、は日本語を主体としたIME（Input Method Environment)である。

このアプリケーションは冨樫雅人氏作成の日本語IME「風」をベースとしつつ、UNICODE対応などの
近代化を図っている。
これによって、風の目指した「文字は自分で選ぶもの」というコンセプトを現代に復活させたいと考えている。

技術的には、風がIMM32ベースの32bitアプリなのに対して、「ひともじ」はTSF（Text Service Framework）
ベースの32bit/64bit両対応アプリとして実装している。

ひともじのリポジトリは以下で公開している。
https://github.com/toshishi-egaoit/hitomoji

なお、ひともじはオープンソースであり、GPLv3ライセンスで配布している。


## 残存バグ

- OnEndEditの処理がうまく動いてない原因を調査
- アイコンが消失(HIDDEN)となる原因の調査
- chatGPTのキャンバスで未確定文字が確定文字となる場合がある
- Excelでデータ制約がリストの場合、選択肢が正しく表示されない

# ロードマップ（2026.5.24版）

現状、v0.5.3までを実装済み。

## hitomoji v0.1系統

目的：
　IMEとしての最低限のキー入力と確定処理が行えること。
　コードは、TSFインタフェース部分とエンジン部の厳密な分離までは求めない。
  これは、分析中毒を防ぐため。

### v0.1 ゴール定義

目的：IMEとしての最小動作確認。入力イベントと Composition の骨格を作る段階。

- IMEのON/OFFができること（Ctrl + Space）
- KeyEventSink の最小限の動作
-- OnTestKeyDown
-- OnKeyDown
-- OnSetFocus(フォーカス変更は未対応)
- ひらがな変換・カタカナ変換は しない
- 変換キーは 存在しない
- Composition の実装
- インライン変換中表示の実装
- 確定キー（Return）による処理
-- 変換中文字列を確定
-- Composition を破棄

### v0.1.1 ゴール定義

目的：ローマ字→ひらがなの最低限の流し込み。

- ひらがな変換マップの作成
- 変換中 Composition に、
-- アルファベット → ひらがな変換結果を与える
- ひらがな確定キーの実装
- カタカナ確定キーの実装
- 英文字変換は実装しない
- 半角カタカナは実装しない
- カナ入力は実装しない
- 変換キーは 実装しない

### v0.1.2 ゴール定義

目的：ソースの整理

- TSFインタフェース部とエンジン部の分離
- エンジン部は、TSFに依存しない形とし、現状のCContytollerをChmEngineに改名
- TSFインタフェース部は現状のCHitomojiをChmTsfIfにリネームしてファクタリング
- DoEditSessionのエンジン依存処理を切り出して、ChmEngineに移動
- 機能追加は行わないが、バグ修正と、TSF対応アプリの拡充は行う(Edgeをターゲットとする)

### v0.1.3 ゴール定義

目的：英字入力系の分離と整理。

- （半角）英文字確定キーの実装	
- 大文字入力サポート
- 記号類の入力サポート
- 実入力されたアルファベットの内部記録が必要
- そのため v0.1.1 では除外
- 全角英文字変換キーの実装

### v0.1.4 ゴール定義

目的：カナ入力、英字入力の精密な定義

- 大文字と小文字の識別（CapsLockも含めて）
- カナ変換時のウ゛の扱い、UNICODE定義されているカナの扱い
- ワ行のい、えの扱い
- てぃ、でぃ、じぇ、などの扱い

### v0.1.5 ゴール定義

目的：編集キーの充実

- ESCによるキャンセル処理(v0.1.4で実装済み)
- BSによる文字削除処理
- v0.2で実装予定のIME個人設定用クラスを作り、BSの挙動を変更できるようにする。
  -- BSを押すと、かな1文字分の削除か、アルファベット１文字削除か選択できる。
  -- DisplayModeとして、未確定文字をかな表示するか、アルファベット表示するか選択できる。
- 矢印キーを押すとかな確定した上でカーソルキーをアプリに渡す
- マウスクリックによる挿入ポイント移動もカーソルキーと同様とする。
- notepadとVS2022、Explorerを目標として、IMEとしての機能が動作することを検証
- Compositionの確定後にのカーソル位置を正しく設定する。
　notepadなど素直なアプリには対応済み
- 以下は当初実装予定だったが、不要と判断し実装しないこととする。
　-- 矢印キーの処理
　-- INSによる挿入モード

### v0.1.6 ゴール定義

目的：Windows依存の処理を実装し、配布可能なパッケージ化する

- この時点では、致命的でない不具合はOKとする。
- 確定後のカーソル位置をただしく設定する。（入力中の表示バグは許容）
- 「ひともじ」アイコンの作成。アイコンはひともじロゴとしての「ひ」。
- ただし、IME ON/OFFの２パターン「A」と「あ」のステータス表示は後送りとする。
- 32ビット版(x86)のビルドを行い動作確認をする。
- インストーラ（64/32の両方が動作するように）を作成。

## v0.2系統

目的：環境設定（個人設定）が行えるようになる。

- 環境ファイル（.ini形式）の作成
-- LoadFileの実装
- 環境情報管理クラスの作成
-- TIP::Activateでオブジェクト作成を行う
-- Getterの実装
-- サンプルとしてのHitomoji.iniの作成

### v0.2.1

目的：利用者の利便性向上と、エラーハンドリング強化

- エラーをログファイルに出力
  ChmLoggerクラスの実装
- LoadFileのエラーハンドリング強化
- バージョン確認コマンド(CTRL+SHIFT+V)の実装（デバッグ版のみ）

### v0.2.2

- テストプロジェクト(hitomoji-test)の追加
- テスト対象コードだけを分離し、hitomoji-engineプロジェクトを追加
- エラーのログ出力
  ChmLoggerクラスの作成。ただし現状はOutputDebugStringを用いる。
- ローマ字かな変換テーブルの外出し
-- 例外のみを外だしとする。ベースは内部に持たせる

### v0.2.3

目的：キー定義の外出し

- 機能キー定義の外出し
- ChmConfigクラスのテストコードの完成
- config項目へのフラグ追加を予定していたが、iniファイルの[key-table]セクションでほぼ対応できるため、廃止とする。

### v0.2.4

目的：キー定義の外出し2

- @include の実装による、configファイルの役割分担
- KANJIDIC2を用いた、Layer2用の教育漢字用文字データを実装


### v0.2.5

目的：TSFまわりの残件の解消

- IME ON/OFFによるCompositionの確定処理の実装
- 言語バーアイコン表示の再挑戦（v0.1.6であきらめた部分）
	- アイコンは言語バーに表示される。
	- v0.2.5.2では、アイコンが消失(HIDDEN)となる不具合が残っている。
- 言語バーへの独自メニューの追加
-- ファイルOpenは既定のエディタで開く
-ｰ ファイルRereadは、ファイルの再読み込みを行う。
-- ConfigフォルダオープンはエクスプローラでConfigフォルダを開く
-- ChmLoggerの強化。ログファイルへの書きこみをサポート
- コードの整理
-- 機能キー名を実体に合うように変更（CommitKana->Finishなど）
- UIの改善
-- かな確定キーの実装（ENTERは「そのまま確定」となったため。ひらがな確定を作る）


## v0.3系統

目的：漢字データベースの整備
　漢字辞書の編集ツールの作成
　ここでは、ひともじ本体への改修はほぼない見込み
  →このバージョンについては、hitomoji-dict-tools リポジトリに反映

## v0.4系統

目的：漢字変換処理の実装

- v0.3で作成した辞書を利用して、漢字変換が行えるようにする。
- 性能検証（辞書をフル実装した場合）

## v0.4

目的：漢字変換処理のコアの実装

- SPACEキーによる変かん状態への遷移の実装
- 辞書の検索ロジックの実装
- キー入力と漢字の選択ロジックの実装(一覧表示なし)

### v0.4.1
### v0.4.2

目的：Undo機能の実装

- 不具合が多く、v0.4.3で再実装することとしたため、仕切り直し

### v0.4.3

目的：Undo機能の実装

- 確定キーを推した直後のみ、Undoキー(CTRL+Z)によって、入力中モードに戻す機能の実装
- 複数回のUndoはサポートしない
- フォーカス移動後/アプリ切り替え後のUndはサポート外だが、アプリによっては動作するのが現状。
- 標準エディットコントロールではUndoが動作しない。これは標準エディットのTSF互換性問題によるものと思われる。
- notepad、Edge、VS2022などでは動作確認済

### v0.4.3.1

バグ修正、仕様追加
- 入力中のUndoは無視する仕様とする。
- 確定後にカーソル移動やBSなどの編集をした場合はUndo無効とする。

### v0.4.3.2

バグ修正、機能追加
- 漢字辞書のSlotに関するバグ修正に伴い、変換エンジン側も修正
- 漢字変換時の選択キーをConfigで指定可能とした。

## v0.5

目的：選択ウィンドウの表示

### v0.5.0

目的：選択ウィンドウ表示スレッドの実装

- この処理は専用スレッドで実装。
- 漢字の候補の一覧ウィンドウを保持するためのスレッドを生成
- このバージョンでは、空ウィンドウを適切な位置（入力キャレットの直下）に表示する。

### v0.5.1

目的：選択ウィンドウのコンテンツ表示

- 漢字の候補を一覧表示するウィンドウの実装
- ページ送りの実装
- エラーハンドリング(ビープ音の実装)
- 表示遅延機能の実装(変換キーを押してから、configでの定義時間が経過してから表示する)

### v0.5.2

目的：辞書の改善

- 訓読みでの濁音化(事=ごと)の実装(辞書の改修が必要)
- ぱ行の読みの追加(辞書の改修が必要)
- 音便による特殊読み(文=も)の追加(辞書の改修が必要)
- 促音で終わる語の対応(辞書とDLL側の改修が必要)

### v0.5.3

目的：アプリ側のモード変更の要せいに対応

- vimなどで、モード変更の通知によりIMEの状態変更する機能の実装
- ブラウザでのパスワード入力などでのIMEのON/OFF制御に対応
- 一部アプリで、想定外のOnSetFocusが呼ばれていた。その際のエラーハンドリングの実装
- DbgViewでの性能劣化対応
- デフォルトのselect-keymapを40文字から34文字に減らす。押しにくい最上段を避けるため。
- ChmEngine側のテストコードの充実

### v0.5.4

目的：第一水準漢字の残りの追加と、細かいバグの修正

- JISの第一水準のうち、常用外、人名用外の漢字を追加。
- 全角スペースの入力
- 変換キーを押した時に、先頭の記号類を無視して変換候補を探す仕様の拡充。
- 複数の読みを指定する方式（風の：指定相当の機能）
- UIの改善: 先頭ページでのBSや末尾ページでのSPACEを無視せず、サイクリックにまわす。
- ブラウザのパスワード欄でIME offにならない。(バグ)
- Excelでリスト形式のセルの場合、1文字入力した時点でdropdownされるはずなのに、出ない。(バグ)

### v0.5.5

目的：候補ウィンドウのカスタマイズ対応

- 候補ウィンドウのフォント指定、サイズ指定の実装
- 候補ウィンドウを見やすくするための改善。ページ番号表示、TやYの位置の視認性改善。
- 表示位置が微妙にずれることがあるのを修正


### v0.5.6

目的：カスタマイズ項目の強化、付加機能の強化、ソースコードの整理

- 入力時の記号類の全角化(configでON/OFFに加えて、切り換え機能キー(WideLock)も作る)
- Layer2でよく使う約物をiniに追加
- 機能キーのconfig対応もれを追加
- ソースコード全体の整理。特にAIとのペアプロで見直しできていない部分の整理。


## v0.6系統

目的：デッドキーを含めた、欧米向きの第二層の確定入力など、漢字変換以外の機構を作る

## v0.6.0

目的：先回り置換機能（Symbol Expander）の実装

- 変換中の文字列に対して、確定前に置換を行う機能
- ローマ字カナ変換とは独立に実装するが、同じエンジンを利用し、サブクラス化して実装する
- 先回り置換の複数エンジンの評価順序をサポートするMgrクラスを実装する
- いわばローマ字変換と同様にU+2xxxあたりの記号をASCIIで入力できるようにする
- 欧米向けに→記号、数学記号、ギリシャ文字などを対象とする
- プレフィックスごとにエンジンを切り替え可能な構造とする。
-- 例:/uxxxxでのUnicode直接入力エンジン、/gXでギリシャ文字入力、/cでキリル文字入力など
- 非日本語圏での配布を考慮し、ローマ字変換などはMgrのconfigでOFFできるようにする。
- Discordユーザをターゲットとして、Layer2の絵文字入力をサポートする。
- Layer2を即確定するUIと、compositionを維持するUIの両方をサポートし、エンジンによって切り替え可能とする。
- 想定されるエンジンは以下の通り。
-- Unicode直接入力エンジン(プレフィックスが+)
-- 即確定絵文字入力エンジン(プレフィックスが:)
-- 即確定記号入力エンジン(プレフィックスが/)
-- composition維持型入力エンジン(プレフィックスなし)
-- composition維持型かな入力エンジン(プレフィックスなし)

## v0.7系統

目的: JIS X 0213の対応(第二水準以降の漢字の追加)

- 辞書のトラッキングが行えるように、メタ情報をヘッダに追加。
- 第二水準以降のslot決定ロジックの実装(dict-tools側)
- 読みがわからない(いわゆる、よみしらず)の扱いの検討。
- 漢字の属性を指定する方式（部首やカテゴリ、教育漢字配当年次指定など）
- ユーザ辞書のサポート(configで定義できる :=123...のように文字列定義することで通常辞書を上書き検索)

## v0.8系統

目的：Layer1の多様化実装

- 物理キーとのマッピングを行うLayer1の実装
- マッピング情報はconfigで定義する
- 複数のマッピング切り換えをサポートする
- 実装予定のレイアウトは以下の通り。
- 101/102キーレイアウト
- 106/109キーレイアウト(日本語キーボード))
- 月配列(2-255?)
- 花配列
- M式キーボード


## v0.9系統

目的：Layer3の複数エンジン対応

- 絵文字変換機構の構想
- 単語変換の機構の実装
- 非漢字ブロックの入力支援(UNICODEのArrowブロックを選択できるUIなど)


## v1.0系統

目的：コードの整理とIMM32対応（必要であれば）
 
- エンジン部分の整理、MacOSやLinuxへの移植を見据えたコードの整理


## v1.0.1 以降の予定

目的：漢字変換の付加機能の実装

- L3用の辞書メンテツールを作成。
- 短文変換機能の実装
- 異体字対応
- 読みの逆引き対応
